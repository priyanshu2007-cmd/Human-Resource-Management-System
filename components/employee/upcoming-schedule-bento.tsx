...
export function UpcomingScheduleBento({ events = DEFAULT_EVENTS }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm dark:shadow-none flex flex-col justify-between transition-all h-full">
      <div>
        {/* Card Header */}
        <div className="flex items-center justify-between pb-4 border-b border-neutral-100 dark:border-slate-800 mb-4">
        ...
        <div className="space-y-3">
          {displayEvents.map((evt) => {
            ...
            return (
              <div
                key={evt.id}
                className="flex items-center justify-between p-3 rounded-2xl border border-neutral-100 dark:border-slate-800/80 bg-neutral-50/50 dark:bg-slate-800/40 hover:bg-neutral-100/70 dark:hover:bg-slate-800/80 transition-colors"
              >
                {/* Date Badge */}
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-700 flex flex-col items-center justify-center shrink-0 shadow-2xs">
                    <span className="text-[10px] font-mono uppercase font-bold text-indigo-600 dark:text-indigo-400 leading-none">
                      {dateObj.month}
                    </span>
                    <span className="text-base font-extrabold font-mono text-neutral-900 dark:text-slate-50 leading-none mt-0.5">
                      {dateObj.day}
                    </span>
                  </div>
                  ...
                  <div className="min-w-0">
                    <p className="text-body-sm font-bold text-neutral-900 dark:text-slate-100 truncate">
                      {evt.title}
                    </p>
