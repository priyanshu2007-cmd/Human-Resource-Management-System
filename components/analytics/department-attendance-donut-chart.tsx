    ...
    >
      <div>
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-title-md font-bold tracking-tight text-neutral-900 dark:text-slate-50">Department Attendance</h3>
          <span className="font-mono text-[10px] uppercase px-2 py-0.5 rounded-full bg-neutral-100 dark:bg-slate-800 text-neutral-500 font-semibold tracking-widest">
            Today
          </span>
        </div>
      ...
      <div className="relative h-[220px] w-full my-2">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="text-3xl font-bold font-mono tracking-tight text-neutral-900 dark:text-slate-50">{overallRate}%</span>
      ...
      {/* Legend */}
      <div className="space-y-1.5 pt-3 border-t border-neutral-200 dark:border-slate-800 text-xs max-h-[90px] overflow-y-auto">
        {chartData.map((d) => (
          <div key={d.name} className="flex items-center justify-between py-1">
            <div className="flex items-center gap-2 truncate">
              <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: d.color }} />
              <span className="truncate text-neutral-600 dark:text-slate-300 font-medium">{d.name}</span>
            </div>
            <span className="font-mono font-bold shrink-0 ml-2 text-neutral-900 dark:text-slate-50">{d.value}%</span>
          </div>
        ))}
      </div>
    </div>
